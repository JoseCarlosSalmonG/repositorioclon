...
class LocalsControllerTest < ActionController::TestCase
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post :create, local: { address: @local.address, city_id: @local.city_id, latitude: @local.latitude, longitude: @local.longitude, name: @local.name }
    end
  ...
  test "should update local" do
    patch :update, id: @local, local: { address: @local.address, city_id: @local.city_id, latitude: @local.latitude, longitude: @local.longitude, name: @local.name }
    assert_redirected_to local_path(assigns(:local))
  end
